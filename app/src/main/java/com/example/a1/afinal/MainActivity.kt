...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        FloatingAction.setOnClickListener{
            val intent = Intent(this, Add_new_contact::class.java)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if (requestCode == ADD_TASK_REQUEST) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                val p_title = data?.getStringExtra("post_id")
                val p_body = data?.getStringExtra("post_name")
                val p_mobile_number = data?.getStringExtra("post_mobile_number")
                val p_home_number = data?.getStringExtra("post_home_number")

                postsList.add(Post(userId, null, p_title, p_body, p_mobile_number))
